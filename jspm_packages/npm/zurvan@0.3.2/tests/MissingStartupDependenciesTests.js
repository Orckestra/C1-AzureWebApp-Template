  ...
  return function(done) {
    ...
    var overriders = globalDependencies.map(function(dependency) {
	  ...
	  assert.throws(function() {
	    ...
	    return globalDependencies.map(function(dependency) {
	      return new RegExp("Missing dependency: " + dependency.name).test(err);
...
describe('zurvan', function() {
  describe('when required', function() {
  	it('throws if there is no setImmediate in environment', startupDependencyTestCase([{name: "setImmediate"}]));
	  it('throws if there is no setImmediate and clearImmediate in environment', startupDependencyTestCase([{name: "setImmediate"}, {name: "clearImmediate"}]));
  });
});