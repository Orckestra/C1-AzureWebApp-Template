...
describe('zurvan with globally configured time to 2015-09-01T00:00:00.000Z', function() {
  it('without additional configuration returns Date() = 2015-09-01T00:00:00.000Z', function() {
  ...
  it('with additional local configuration global settings are overridden, but only for this single interception period', function() {
    return zurvan.interceptTimers({systemTime: "2010-01-05T05:55:11.158Z"}).then(function() {
  ...
  it('does not interfere with other options set on lower level', function() {
    return zurvan.interceptTimers({timeSinceStartup: 60}).then(function() {
	  assert.equal(new Date().toISOString(), "2015-09-01T00:00:00.000Z");
